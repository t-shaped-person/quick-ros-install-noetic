...
if [ -f "/etc/udev/rules.d/97-omo-r1-ydlidar.rules" ]; then
    ...
    echo 'SUBSYSTEM=="tty", KERNELS=="3-3", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", GROUP:="dialout",  SYMLINK+="ydlidar"' >/etc/udev/rules.d/97-omo-r1-ydlidar.rules
